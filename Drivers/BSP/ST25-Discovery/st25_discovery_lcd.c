...
#if defined(USE_M25R16B)
    /****************************************/
    /*       Display Invertion ON           */
    /****************************************/
    LCD_IO_WriteReg(LCD_DINVON);
  
    /****************************************/
    /*       Memory Access Control          */
    /****************************************/
    LCD_IO_WriteReg(LCD_MAC);
    LCD_IO_WriteData(0x68);
#endif /* USE_M25R16B */
...
/**
  ...
  * @retval None
  */
void BSP_LCD_DrawPixel(uint16_t Xpos, uint16_t Ypos, uint16_t RGB_Code, uint8_t orientation)
{
  if(LcdDrv->WritePixel != NULL)
  {
    LcdDrv->WritePixel(Xpos, Ypos, RGB_Code, orientation);
  }
}
...
void BSP_LCD_DrawHLine(uint16_t Xpos, uint16_t Ypos, uint16_t Length)
{
  ...
  else
  {
    ...
    {
      BSP_LCD_DrawPixel((Xpos + index), Ypos, DrawProp.TextColor, 2);
    }
  }
}
...
void BSP_LCD_DrawVLine(uint16_t Xpos, uint16_t Ypos, uint16_t Length)
{
  ...
  else
  {
    for(index = 0; index < Length; index++)
    {
      BSP_LCD_DrawPixel(Xpos, Ypos + index, DrawProp.TextColor, 2);
    }
  }
}
...
void BSP_LCD_DrawLine(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
  ...
  {
    BSP_LCD_DrawPixel(x, y, DrawProp.TextColor, 2);  /* Draw the current pixel */
...
void BSP_LCD_DrawCircle(uint16_t Xpos, uint16_t Ypos, uint16_t Radius)
{
  ...
  while (CurX <= CurY)
  {
    BSP_LCD_DrawPixel((Xpos + CurX), (Ypos - CurY), DrawProp.TextColor, 2);

    BSP_LCD_DrawPixel((Xpos - CurX), (Ypos - CurY), DrawProp.TextColor, 2);

    BSP_LCD_DrawPixel((Xpos + CurY), (Ypos - CurX), DrawProp.TextColor, 2);
...
void BSP_LCD_DrawEllipse(int Xpos, int Ypos, int XRadius, int YRadius)
{
  ...
  do {      
    BSP_LCD_DrawPixel((Xpos-(uint16_t)(x/K)), (Ypos+y), DrawProp.TextColor, 2);
    BSP_LCD_DrawPixel((Xpos+(uint16_t)(x/K)), (Ypos+y), DrawProp.TextColor, 2);
    BSP_LCD_DrawPixel((Xpos+(uint16_t)(x/K)), (Ypos-y), DrawProp.TextColor, 2);
    BSP_LCD_DrawPixel((Xpos-(uint16_t)(x/K)), (Ypos-y), DrawProp.TextColor, 2);
...
void BSP_LCD_DrawRectangleWithRoundCorner(uint16_t Xpos, uint16_t Ypos, uint16_t Width, uint16_t Height, uint16_t Radius)
{
    uint16_t r = Radius;
...
void BSP_LCD_DrawTabHeader(uint16_t Xpos, uint16_t Ypos, uint16_t Width, uint16_t Height, uint16_t Radius, uint16_t tabHeight, uint32_t tabId, uint32_t nbTab)
{
    uint16_t r = Radius;
...
void BSP_LCD_DrawTab(uint16_t Xpos, uint16_t Ypos, uint16_t Width, uint16_t Height, uint16_t Radius, uint16_t tabHeight, uint32_t tabId, uint32_t nbTab)
{
    uint16_t r = Radius;
    uint16_t textColor = DrawProp.TextColor;
//    uint16_t backColor = DrawProp.BackColor;
...
void BSP_LCD_WriteBMP_FAT(uint16_t Xpos, uint16_t Ypos, const char* BmpName)
{
  ...
  height = BmpName[1]+1; 
    ...
    BSP_LCD_DrawPixel( ILI9341_NO_CURSOR, ILI9341_NO_CURSOR, data ,2);
		pBmpWord++;
	} 
...
static uint8_t BSP_LCD_Write24BPixelLine(uint8_t* Row, uint32_t DataLength)
{
  ...
  for(counter = 0; counter < width; counter++)
  {
    ...
    if(enableGreyScale)
    {
      ARGB32Buffer[crop_width]  = BSP_LCD_ConvertToGreyScale(RGB_matrix[counter].R, RGB_matrix[counter].G,RGB_matrix[counter].B);
...
/**
  * @brief  Display cropped jpeg picture file on LCD.
  * @param  PictPos: specifies the X,Y picture position on LCD (in pixels).
  * @param  PictCropPos: specifies the X,Y cropped origin (in pixels).
  * @param  CropWidth: specifies the cropped picture width (in pixels).
  * @param  CropHeight: specifies the cropped picture height (in pixels).
  * @Param  Pict: pointer to the picture data.
  * @retval None
  */
uint32_t BSP_LCD_DisplayCropJpeg(pPoint PictPos, pPoint PictCropPos, uint16_t CropWidth, uint16_t CropHeight, const char* Pict)
{
  uint32_t w,h;
  ...
  if(IS_JPEG(Pict))
    ...
    /* Set default values to the crop settings. */
    crop.factor = 1;
    crop.first_row = PictCropPos->Y;
    crop.first_col = PictCropPos->X;
    crop.last_col = PictCropPos->X + CropWidth;
    crop.last_row = PictCropPos->Y + CropHeight;
    crop.line_counter = 0;
    
    BSP_LCD_SetDisplayWindow(PictPos->X, PictPos->Y, CropWidth-1, CropHeight-1);

    // Display on LCD
    jpeg_decode(Pict,BSP_LCD_Write24BPixelLine);
...
static void DrawChar(uint16_t Xpos, uint16_t Ypos, const uint8_t *c)
{
  ...
  uint32_t line;
  uint16_t *pBuff;
// Font limit for this function: font36 36 pixels x 25 pixels
  uint16_t buffer[36*25];
